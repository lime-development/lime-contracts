    ...
    UUPSUpgradeable
{
    ...
    function mint(address to, uint256 amount) public {  
        require(((pairedToken!=address(0))&&(pool!=address(0))), "The token must be initialized");
        ...
        require(IERC20(pairedToken).balanceOf(msg.sender) >= withdrow, "Insufficient token balance");
        require(IERC20(pairedToken).allowance(msg.sender,address(this)) >= withdrow, "No token allowance has been issued");
    ...
    function calculatePrice(uint256 amount) public view returns (uint256 poolAmount, uint256 protocolFee) {
        poolAmount = calculateValue(totalSupply()+amount)-calculateValue(totalSupply());
        protocolFee = poolAmount * 3 / 100;
    }
    ...
    function calculateValue(
        ...
        if(externalDecimals>decimals()){
            _price = (amount * amount / 2)*10**(externalDecimals-decimals());
        } else {
            _price = (amount * amount / 2)/10**(decimals()-externalDecimals);
        }
        return _price;
    }
